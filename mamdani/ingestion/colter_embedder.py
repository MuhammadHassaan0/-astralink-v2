"""
ingestion/colter_embedder.py
Embeds chunks from colter_chunks.json using FastEmbed (dense) + BM25 (sparse),
then upserts into a Qdrant collection called 'colter'.
"""
...
CHUNKS_FILE     = Path(__file__).parent / "raw_data" / "colter_chunks.json"
...
def sparse_vector_for(tokens: list[str], vocab: dict[str, int], idf: dict[str, float],
...
def main():
    ...
    sparse_vecs = [sparse_vector_for(tok, vocab, idf, avg_dl=avg_dl) for tok in tokenised]
