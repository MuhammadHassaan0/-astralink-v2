"""
ingestion/faizah_chunker.py
Reads faizah_scraped.json, chunks each document into 200-350 token pieces,
attaches metadata, and saves to raw_data/faizah_chunks.json.
"""
...
INPUT_FILE  = Path(__file__).parent / "raw_data" / "faizah_scraped.json"
OUTPUT_FILE = Path(__file__).parent / "raw_data" / "faizah_chunks.json"

TODAY = date.today().isoformat()

# ── Topic keywords (LA CD11 specific) ────────────────────────────────────────
TOPIC_KEYWORDS = {
    "housing":       ["housing", "rent", "tenant", "landlord", "affordable", "eviction",
                      "apartment", "home", "shelter", "rso", "rent stabilization",
                      "venice dell", "public counsel", "ellis act", "just cause"],
    "homelessness":  ["homeless", "unhoused", "housing-first", "housing first",
                      "encampment", "shelter", "supportive housing", "la hope",
                      "inside safe", "street medicine"],
    "climate":       ["climate", "environment", "wildfire", "fire", "coastal",
                      "sea level", "venice beach", "pacific palisades", "palisades",
                      "clean energy", "emissions", "sustainability", "green"],
    "government":    ["transparency", "accountability", "accessible", "responsive",
                      "constituent", "community", "budget", "planning", "council",
                      "democracy", "grassroots", "organizing"],
    "public_safety": ["safety", "crime", "police", "lapd", "violence",
                      "community safety", "gun", "mental health"],
}
...
def main():
    print("=== Faizah Chunker ===")

    docs = json.loads(INPUT_FILE.read_text())
    print(f"  Loaded {len(docs)} documents from faizah_scraped.json\n")
    ...
    if total:
        print(f"  is_quote=true:    {n_quote}  ({n_quote/total*100:.1f}%)")
        print(f"  is_recent=true:   {n_recent}  ({n_recent/total*100:.1f}%)")
    print(f"\n  By topic:")
