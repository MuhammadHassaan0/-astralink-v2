"""
retrieval/faizah_retriever.py
Hybrid retrieval over the Qdrant 'faizah' collection using:
...
def _load_vocab_idf():
    ...
    if _vocab is not None:
        return
    chunks_file = Path(__file__).parent.parent / "ingestion" / "raw_data" / "faizah_chunks.json"
    import json
...
QUERY_PATTERNS: dict[QueryType, list[str]] = {
    "recent_event":   ["latest", "today", "recent", "this week", "2026", "2025",
                       "announced", "new", "just", "now", "update", "fire", "palisades"],
    "policy_issue":   ["housing", "rent", "tenant", "eviction", "homeless", "unhoused",
                       "climate", "wildfire", "safety", "budget", "plan", "policy",
                       "affordable", "venice dell", "rso", "public counsel", "district 11"],
    "persona_general": ["who", "believe", "think", "feel", "values", "why",
                        "stand for", "care about", "vision", "philosophy",
                        "background", "experience", "why running"],
...
TOPIC_MAP = {
    "housing":      ["housing", "rent", "tenant", "eviction", "affordable", "venice dell", "rso"],
    "homelessness": ["homeless", "unhoused", "encampment", "housing-first"],
    "climate":      ["climate", "wildfire", "fire", "coastal", "environment", "palisades"],
    "government":   ["transparency", "accountability", "budget", "council", "responsive"],
    "public_safety": ["safety", "crime", "police", "violence"],
}
...
if __name__ == "__main__":
    queries = [
        ...
        "What happened with the Venice Dell project?",
    ]
    for q in queries:
        print(f"\nQuery: {q!r}")
